...
class SSH :
    ...
    /**
     * Opens a new SFTPv3Client on the current authenticated connection.
     * Must only be called after authentication succeeds.
     * The caller is responsible for closing the returned client.
     *
     * @return a ready-to-use [com.trilead.ssh2.SFTPv3Client], or null if not connected/authenticated.
     */
    fun openSftpClient(): com.trilead.ssh2.SFTPv3Client? {
        val conn = connection ?: return null
        if (!authenticated) return null
        return try {
            com.trilead.ssh2.SFTPv3Client(conn)
        } catch (e: Exception) {
     ...
     * Waits for the command to finish and returns its exit code (null if unavailable).
     * Opens and closes its own session — does not affect the main terminal session.
     *
     * @param command The shell command to execute.
     * @return The exit status code, or null if it could not be determined.
     */
    fun execCommand(command: String): Int? {
        val conn = connection ?: return null
        if (!authenticated) return null
        return try {
            val execSession = conn.openSession()
            try {
                execSession.execCommand(command)
                // Drain stdout/stderr to avoid blocking the server
                execSession.stdout?.copyTo(java.io.OutputStream.nullOutputStream())
                execSession.stderr?.copyTo(java.io.OutputStream.nullOutputStream())
                execSession.waitForCondition(
                    com.trilead.ssh2.ChannelCondition.EXIT_STATUS,
                    30_000,
                )
                execSession.exitStatus
            } finally {
                execSession.close()
            }
        } catch (e: Exception) {
            Timber.e(e, "execCommand failed: $command")
            null
        }
    }
